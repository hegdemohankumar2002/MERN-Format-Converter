...
const HEVCToMP4 = () => {
  ...
  const handleFiles = (fileList) => {
    const arr = Array.from(fileList).filter(f => /\.(hevc|mkv|mov)$/i.test(f.name));
    setFiles(arr);
  };
  ...
  const handleUpload = async () => {
    if (!files.length) {
      toast.warn("❌ Please select at least one HEVC file to convert.");
      return;
    }
  ...
  return (
    <div className="neon-card" style={{ maxWidth: 420, margin: "2.5rem auto", position: "relative" }}>
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <h2 className="text-neon" style={{ marginBottom: 6, textShadow: 'none' }}>HEVC to MP4</h2>
        <p style={{ color: "#b3b3ff", marginBottom: 18 }}>Convert HEVC videos to MP4 format.</p>
      </div>
      {/* Quota info banner */}
      ...
      >
        ...
        <span style={{ color: dragActive ? "var(--color-accent)" : "#b3b3ff" }}>
          {dragActive ? "Drop files here!" : "Drag & drop HEVC files or click to select"}
        </span>
      </div>
      {files.length > 0 && (
      ...
      {downloadUrl && (
        <a
          href={`${process.env.REACT_APP_BACKEND_URL || `${window.location.protocol}//${window.location.hostname}:5000`}${downloadUrl}`}
          target="_blank"
