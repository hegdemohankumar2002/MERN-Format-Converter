...
const RAWToJPG = () => {
  ...
  const handleFiles = (fileList) => {
    const arr = Array.from(fileList).filter(f => /\.(cr2|nef|arw|dng|orf|raw|rw2|raf|sr2)$/i.test(f.name));
    setFiles(arr);
  };
  ...
  const handleUpload = async () => {
    if (!files.length) {
      toast.warn("❌ Please select at least one RAW file to convert.");
      return;
    }
    ...
    try {
      ...
      if (err.response && err.response.status === 403) {
        await fetchQuota(); // Ensure quota is updated before showing toast
        toast.error("You have reached your free conversion limit. Please register or log in for unlimited conversions.");
      } else {
        await fetchQuota();
  ...
  return (
    <div className="neon-card" style={{ maxWidth: 420, margin: "2.5rem auto", position: "relative" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <h2 className="text-neon" style={{ marginBottom: 6, textShadow: 'none' }}>RAW to JPG</h2>
        <p style={{ color: "#b3b3ff", marginBottom: 18 }}>Convert RAW camera files to standard JPG images.</p>
      </div>
      <div
      ...
      >
        <input
          ...
          type="file"
          multiple
          accept=".cr2,.nef,.arw,.dng,.orf,.raw,.rw2,.raf,.sr2"
          style={{ display: "none" }}
        ...
        <span style={{ color: dragActive ? "var(--color-accent)" : "#b3b3ff" }}>
          {dragActive ? "Drop files here!" : "Drag & drop RAW files or click to select"}
        </span>
      </div>
      {files.length > 0 && (
