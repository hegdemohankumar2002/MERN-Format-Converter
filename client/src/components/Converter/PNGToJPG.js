...
const PNGToJPG = () => {
  ...
  const handleFiles = (fileList) => {
    const arr = Array.from(fileList).filter(f => /\.png$/i.test(f.name));
    setFiles(arr);
  };
  ...
  const handleUpload = async () => {
    if (!files.length) {
      toast.warn("❌ Please select at least one PNG file to convert.");
      return;
    }
  ...
  return (
    <div className="neon-card" style={{ maxWidth: 420, margin: "2.5rem auto", position: "relative" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <h2 className="text-neon" style={{ marginBottom: 6, textShadow: 'none' }}>PNG to JPG</h2>
        <p style={{ color: "#b3b3ff", marginBottom: 18 }}>Convert PNG images to JPG format easily.</p>
      </div>
      <div
      ...
      >
        ...
        <span style={{ color: dragActive ? "var(--color-accent)" : "#b3b3ff" }}>
          {dragActive ? "Drop files here!" : "Drag & drop PNG files or click to select"}
        </span>
      </div>
      {files.length > 0 && (
